 ...
 * 所谓编译执行,就是通过即时编译器(Just In Time, JIT)将字节码转换为本地机器码来执行;现代JVM会根据代码热点生成相应的本地机器码.
 * <p>
 * 基于栈的指令集与基于寄存器的指令集之间的关系:
 * 1. JVM执行指令时所采取的方式是基于栈的指令集.
 * 2. 基于栈的指令集主要的操作有入栈与出栈两种.
 * 3. 基于栈的指令集的缺点在于完成相同的操作,指令数量通常要比基于寄存器的指令集数量要多;基于栈的指令集是在内存中完成操作的,而基于寄存器的指令集是直接由CPU来执行的,它是在高速缓存区中执行的,速度要快很多.
